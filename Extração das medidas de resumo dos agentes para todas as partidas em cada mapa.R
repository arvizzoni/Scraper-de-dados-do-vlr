...
mapas=c("Ascent","Bind","Breeze","Fracture",'Haven',"Icebox","Lotus","Pearl","Split")
mapas_codigos_url=c("5","1","8","9",'2',"6","11","10","3")
...
tabelas_com_os_dados_por_mapa=list()
for(mapa in c(1:length(mapas))){
  tabelas_com_os_dados_de_agentes=list()
  for(agente in c(1:length(agentes))){
    url_dos_dados=paste0(radical,'map_id=',mapas_codigos_url[mapa],'&agent=',agentes_codigos_url[agente],sufixo)
  ...
  tabelas_com_os_dados_por_mapa[[mapa]]=tabelas_com_os_dados_de_agentes
}
...
for(mapa in c(1:length(mapas))){
  medidas_de_resumo_dos_agentes=lapply(tabelas_com_os_dados_por_mapa[[mapa]],FUN=funcao_para_medidas_de_resumo)
  ...
  medidas_de_resumo_dos_agentes=cbind(mapas[mapa],data.frame(medidas_de_resumo_dos_agentes))
  colnames(medidas_de_resumo_dos_agentes)[1:2]=c('Mapa','Agente')
  
  #Arquivo com os dados de agentes para amostras de todas as partidas já jogadas
  write.csv2(medidas_de_resumo_dos_agentes,file=paste0(Sys.Date(),' - Medidas de resumo dos agentes - Todas as partidas - Mapa=',mapas[mapa],'.csv'))
}